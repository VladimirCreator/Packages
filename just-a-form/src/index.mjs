import './global.css'
import { rulesForFirstName, rulesForLastName, rulesForBirthday, rulesForEmail } from './rules.mjs'
...
const controller = {
	launch() {
		view.form.addEventListener('input', event => event.target.setCustomValidity(''))
		...
		view.submit.addEventListener(
			'click', () => {
				...
				if (hasProblem) {
					view.form.noValidate = false; view.form.reportValidity()
				}
				else view.form.submit()
			}
		)
	},
	/**
	...
	avoids(conditions) {
		...
		for (let i = 0; i < conditions.length; ++i) {
			if (conditions[i].args) {
				checks.push(
					conditions[i].rule.apply(
						null,
						conditions[i].args.concat(this.value).reverse()
					)
				)
			}
			else
			...
			if (checks.includes(true)) {
				this.setCustomValidity(conditions[i].description); break
			}
		}
