	...
	internal var body: some View {
		ScrollView {
			VStack(spacing: 8.0) {
				Card {
					InfoSection(
						name: bookingViewModel.booking.hotelName,
						adress: bookingViewModel.booking.hotelAdress,
						literal3: (value: bookingViewModel.booking.horating, name: bookingViewModel.booking.ratingName)
					)
						.frame(maxWidth: .infinity, alignment: .leading)
				}
				Card { Cell5(info: info).frame(maxWidth: .infinity) }
				Card { Cell6(consumer: $bookingViewModel.consumer) }
				Card {
				...
				Card { Cell4(totalPrice: totalPrice, items: items) }
			}
		}
